...
class DatabaseManager:
    ...
    async def create_or_update_user_points(self, user_id, points_to_add=0.0):
        """Crea o actualiza los puntos del usuario"""
        async with self.async_session() as session:
            user_points = await session.execute(
                select(UserPoints).where(UserPoints.user_id == user_id)
            )
            user_points = user_points.scalar_one_or_none()

            if user_points:
                user_points.total_points += points_to_add
                user_points.available_points += points_to_add
    ...
    async def use_user_points(self, user_id, points_to_use):
        """Usa puntos del usuario para ver videos"""
        async with self.async_session() as session:
            user_points = await session.execute(
                select(UserPoints).where(UserPoints.user_id == user_id)
            )
            ...
            if not user_points or user_points.available_points < points_to_use:
                return False

            user_points.available_points -= points_to_use
            user_points.used_points += points_to_use
            user_points.last_activity = datetime.now(timezone.utc)
            ...
            transaction = PointsTransaction(
                ...
                description=f'Usado para ver video'
            )
            session.add(transaction)
            await session.commit()
            return True

    async def create_referral(self, referrer_id, referral_code=None):
        """Crea un código de referido para el usuario"""
        if not referral_code:
            # Generar código único
            referral_code = secrets.token_hex(8).upper()[:16]
        ...
        async with self.async_session() as session:
            ...
            if existing.scalar_one_or_none():
                # Generar uno nuevo si ya existe
                referral_code = secrets.token_hex(8).upper()[:16]
    ...
    async def complete_referral(self, referral_code, referred_user_id):
        """Completa un referido cuando un usuario nuevo se une"""
        async with self.async_session() as session:
            ...
            if not referral or referral.referred_id:
                return False
            ...
            # Otorgar puntos al referrer (5 puntos máximo por día)
            referrer_points = await self.create_or_update_user_points(referral.referrer_id, 5.0)

            # Registrar transacción
            transaction = PointsTransaction(
                user_points_id=referrer_points.id,
                transaction_type='referral',
                amount=5.0,
                description=f'Referido completado: {referral_code}',
    ...
    async def get_referral_by_code(self, referral_code):
    ...
    async def add_points_transaction(self, user_points_id, transaction_type, amount, description, reference_id=None):
    ...
    async def get_points_history(self, user_id, limit=50):
        ...
        async with self.async_session() as session:
            ...
            result = await session.execute(
                ...
                .where(PointsTransaction.user_points_id == user_points.id)
